...
load ('sela_HAR_fullData.mat');
  
rand_mat = randperm(size(X, 1));
trainData_percentage = 93;
for p = 1:ceil((trainData_percentage/100)*length(X))
    X_train{p, :} = X{rand_mat(p)};
    y_train{p, :} = y{rand_mat(p)};
end

for q = 1:(length(X)-ceil((trainData_percentage/100)*length(X)))
    X_test{q, :} = X{rand_mat(length(X)-q + 1)};
    y_test{q, :} = y{rand_mat(length(X)-q + 1)};
end
...
[training_prediction_accuracy, training_confusion_matrix] = sela_predict_HAR( window_size, step_size,... 
                                                            ...
                                                            nnz_w1, nnz_w2);
%figure(2)                                          
[testing_prediction_accuracy, testing_confusion_matrix] = sela_predict_HAR( window_size, step_size,... 
                                                            a, b, X_test, y_test,...
                                                            alpha, beta, gamma,...
                                                            nnx_w1, nnx_w2,...
                                                            nny_w1, nny_w2,...
...
% =========================================================================
training_result(z, :) = training_confusion_matrix(7, :);
testing_result(z, :) = testing_confusion_matrix(7, :);

end

for m = 1:7
    training_result(z+1, m) = mean(training_result(1:z, m));
    testing_result(z+1, m) = mean(testing_result(1:z, m));
end
result(1, :) = training_result(z+1, :);
result(2, :) = testing_result(z+1, :);
save Result_75_25.mat result
%%  END